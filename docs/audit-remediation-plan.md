...
**Status: resolved.** `database/migrations/20260801-baseline-schema.js` — DDL captured verbatim from Sequelize's own `sync()` output against an empty database (not hand-authored), squashing all 40 current model tables into one migration dated before every existing incremental one. Its `up()` also inserts `sequelize_meta` rows for all 22 pre-existing incremental migrations it supersedes, so a fresh `db:migrate` run on an empty database applies the baseline for real and correctly skips re-running the (now-redundant) incremental ones. Verified end-to-end: full `db:migrate` chain against a genuinely empty MySQL database (23/23 migrations applied, 41 tables, spot-checked `tasks.points`/`users.phone` for no duplicate-column errors), plus isolated `up()`/`down()` round-trip on the baseline migration alone. Existing dev DB has it recorded in `sequelize_meta` without re-running (schema already present via `sync()`).
